    ...
    <div className="group glass relative aspect-[3/4] overflow-hidden rounded-xl2 transition-all duration-300 hover:-translate-y-1 hover:shadow-lift">
      ...
      <button
        ...
        className={`absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-full transition-colors ${
          item.fav ? "bg-accent text-canvas shadow-soft" : "bg-white/80 text-ink backdrop-blur-md"
        }`}
      >
        <Heart size={15} />
      </button>
      <button
        ...
        className="absolute left-2 top-2 grid h-8 w-8 place-items-center rounded-full bg-white/80 text-ink opacity-0 backdrop-blur-md transition-all group-hover:opacity-100"
      >
        <Dots size={15} />
      </button>
      ...
      <div className="absolute inset-x-0 bottom-0 px-3 pb-3 pt-14"
        style={{ background: "linear-gradient(to top, rgba(255,255,255,0.95) 58%, rgba(255,255,255,0))" }}>
        ...
        <div className="mt-2 flex items-center justify-between gap-2 border-t border-line pt-2">
          <button
            ...
            title="Log a wear"
            className="flex shrink-0 items-center gap-1 rounded-full px-1.5 py-0.5 text-[11px] font-medium text-muted transition-colors hover:bg-white/70 hover:text-accent-deep"
          >
            <Check size={11} />
          ...
          {editingPrice ? (
            <form onSubmit={savePrice} className="flex min-w-0 items-center gap-1">
              <input
                ...
                onBlur={savePrice}
                className="w-16 rounded-md border border-line bg-white/60 px-1.5 py-0.5 text-[11px] text-ink outline-none focus:border-accent"
              />
            </form>
