...
| Image generation | **Hosted SDXL** (or a fashion-tuned SDXL checkpoint) via **fal.ai** or **Replicate** | Pay-per-generation, zero GPU infra to run/maintain solo; fal.ai tends to be faster/cheaper per image, Replicate has the widest model selection — trial both on real outfit prompts before committing | Self-hosted GPU (e.g. on Modal/RunPod) — revisit once volume is high enough that per-image hosted cost exceeds a dedicated GPU's amortized cost. **Magnific** (formerly Freepik, rebranded April 2026) — also viable: one API (`api.freepik.com`) across 50+ models including their in-house Mystic and Flux, same async task-id/poll pattern as fal.ai/Replicate so it drops into the same background-job architecture with no rework. Pricing is subscription+credit (Premium $14.50/mo for 20K credits, up to Pro at $210/mo) rather than fal.ai/Replicate's pure pay-per-generation — less granular for per-image cost tracking (`07-costs-budget.md`) and you pay for the monthly bucket whether used or not, better suited to steady volume than spiky early-beta usage. Worth trialing alongside fal.ai when Phase 2 starts — same outfit prompts through Mystic/Flux vs. SDXL, compare quality and real cost-per-image before picking. |
